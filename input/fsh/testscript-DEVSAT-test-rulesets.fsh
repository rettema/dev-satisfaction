...
* insert TSMetadata({instance_name}, http://hl7.org/fhir/uv/dev-satisfaction, {date}, "HL7 FHIR R5 TestScript Read Validate {profile_description}", "Example TestScript to test {profile_description} resource read and profile validation")
* insert TSOrigin(1)
...
* insert TSTestAssert("Confirm that the returned response code is 200 OK.", #response, false, false, responseCode, "200")
* insert TSTestAssert("Confirm that the returned response payload is the specified resource.", #response, false, false, resource, #{resource})
* insert TSTestAssert("Confirm that the returned resource conforms to the specified profile.", #response, false, false, validateProfileId, "{profile_id}")
